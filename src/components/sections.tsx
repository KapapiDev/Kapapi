  ...
  return (
    <section className={`${s.sec} ${s.secLine}`} aria-labelledby="flow-h">
      <div className="frame">
        ...
        {/* One continuous record, not three cards. This is a single work item's
            history — splitting it into equal tiles would say "three features"
            when the page is claiming "one thing happened to you". */}
        <Reveal className={s.recordShell}>
          <div className={s.record}>
            <div className={s.band}>
              <div className={s.bandRail}>
                <span className={s.bandNo}>01</span>
                <span className={s.bandLine} aria-hidden="true" />
              </div>
              <div className={s.bandBody}>
                <p className={s.bandKo}>업무를 올립니다</p>
                <p className={s.bandSub}>{q.title}</p>
              ...
              <p className={s.bandMeta}>마감 {q.deadlineHours}시간<span>예산 상한 {won(q.budget)}</span></p>
            </div>

            <div className={`${s.band} ${s.bandMid}`}>
              <div className={s.bandRail}>
                <span className={s.bandNo}>02</span>
                <span className={s.bandLine} aria-hidden="true" />
              </div>
              <div className={s.bandBody}>
                <p className={s.bandKo}>카파피가 배정합니다</p>
                <p className={s.bandSub}>{picked.person.name} · {picked.person.career}</p>
              ...
              <p className={s.bandMeta}>
                <b className={s.bandBig}>{won(picked.bid.price)}</b>
                <span>{picked.bid.hours}시간 완료 제안</span>
              </p>
            </div>

            <div className={s.band}>
              <div className={s.bandRail}>
                <span className={s.bandNo}>03</span>
              </div>
              <div className={s.bandBody}>
                <p className={s.bandKo}>결과가 도착합니다</p>
                <p className={s.bandSub}>{result.files[0].name}</p>
              ...
              <p className={s.bandMeta}>
                <b className={s.bandBig}>{result.at}</b>
                <span>마감보다 {result.earlyMinutes}분 일찍</span>
              </p>
            </div>
          </div>
        </Reveal>
      ...
      <div className="frame">
        ...
        <div className={s.acct}>
          <div>
            ...
            <p className={s.body}>거래 데이터가 신뢰를 만들고, 신뢰가 배정과 복구를 가능하게 합니다. 충분히 검증된 업무부터 카파피가 실행의 중간을 더 많이 맡습니다.</p>
          </div>
          <div className={s.acctBox}>
            ...
            <div className={s.acctRow}><span className={s.acctId}>03</span><span className={`${s.badge} ${s.badgeGm}`}>ROUTE</span><span className={s.acctWhat}>선정 · 배정 · 대체 · 복구</span></div>
  ...
  return (
    <section className={s.sec} aria-labelledby="acct-h">
      <div className="frame">
        ...
        {/* Three roles across one row rather than a fourth left-text/right-panel
            block. The claim is that they hold at the same time, so they sit side
            by side with one account line running under all three. */}
        <div className={s.roles}>
          {[
            { id: "#0182", role: "발주자", what: "내가 맡긴 업무", state: "완료", cls: s.roleClient },
            { id: "#0207", role: "작업자", what: "내가 수행 중인 업무", state: "진행 중", cls: s.roleWorker },
            { id: "#0201", role: "제안 참여", what: "내가 제안을 보낸 업무", state: "대기", cls: s.roleBid },
          ].map((x, i) => (
            <Reveal key={x.id} delay={i * 90} className={`${s.role} ${x.cls}`}>
              <span className={s.roleTag}>{x.role}</span>
              <p className={s.roleWhat}>{x.what}</p>
              <p className={s.roleMeta}><span className={s.roleId}>{x.id}</span>{x.state}</p>
            </Reveal>
          ))}
        </div>

        <Reveal delay={280} className={s.rolesFoot}>
          <p className={s.rolesLine}>세 가지가 같은 계정에서 동시에 성립합니다. 계정을 바꾸거나 다시 가입할 필요는 없습니다.</p>
          <div className={s.cta}>
            <Link href="/board" className={`${s.btn} ${s.btnAccent}`}>작업 찾기<span aria-hidden="true">→</span></Link>
